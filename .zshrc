...
f(){
  local d="${1:-.}"
  if [[ -f "$d" ]]; then
    echo $1 | fzf --preview="bat --color=always --style=numbers --line-range=:500 {}" --bind "enter:become(vim {})"
  elif [[ -d "$d/.git" ]]; then
    git -C "$d" ls-files | fzf --preview="bat --color=always --style=numbers --line-range=:500 $d/{}" --bind "enter:become(vim $d/{})"
  else
    find "$d" -type f -print | fzf --preview="bat --color=always --style=numbers --line-range=:500 {}" --bind "enter:become(vim {})"
  fi
}
